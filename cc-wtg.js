...
//  version 1.4.4
//  2014-01-09
//  Copyright (c) 2009, Daniil Pavliuchkov (https://github.com/dpavliuchkov)
...
const ScriptLink = 'https://userscripts-mirror.org/scripts/source/58929.user.js';
const ScriptMetaLink = 'https://userscripts-mirror.org/scripts/source/58929.meta.js';
...
function gameObject(gameNumber, gameState, tournament, privateGame, speedGame, map, gameType, polySlots, initialTroops, playOrder, bonusCards, fortifications, warFog, trenchWarfare, roundLimit, roundLength, round, timeRemaining, players) {
...
function clearTabContent() {
  ...
  th = document.createElement('th');
  trHeader.appendChild(th);
  th.innerHTML = 'Spoils<br>Reinforcements<br>Special Gameplay';
  th = document.createElement('th');
  trHeader.appendChild(th);
  th.innerHTML = 'Round Limit<br>Round - Time Remaining';
  th = document.createElement('th');
  trHeader.appendChild(th);
  ...
  if (IsInside) {
    ...
    if (/gamelink/.test(this.className)) {
      ...
      if (this.parentNode.childNodes[0].tagName == 'SPAN') {
        ...
        gameNumber = this.parentNode.childNodes[3].innerHTML; // thanks to dgz345 for the fix
      }
    }
...
function addGameRow(game, color) {
  ...
  switch (game.gameType) {
    ...
    case 'P': game.gameType = 'Polymorphic'; break;
  }
  ...
  switch (game.bonusCards) {
    ...
    case '5': game.bonusCards = 'Zombie'; break;
  }
  switch (game.fortifications) {
    ...
    case 'P': game.fortifications = 'Parachute'; break;
    case 'N': game.fortifications = 'None'; break;
  }
  game.roundLimit = game.roundLimit == 0 ? 'None' : game.roundLimit + ' Rounds';
  game.round = 'Round ' + game.round;

  var specialGameplayText = '';
  specialGameplayText += (game.warFog == 'Y') ? 'Fog' : '';
  specialGameplayText += (game.trenchWarfare == 'Y') ? ' Trench' : '';
  if (specialGameplayText == '') specialGameplayText = 'None';
  ...
  var gameTypeText = game.gameType;
  if (game.gameType == 'Polymorphic') gameTypeText += ' (' + game.polySlots + ')';
  tr.insertCell(1).innerHTML = gameTypeText + '<br>' + game.initialTroops + '<br>' + game.playOrder;
  tr.insertCell(2).innerHTML = game.map;
  tr.insertCell(3).innerHTML = game.bonusCards + '<br>' + game.fortifications + '<br>' + specialGameplayText;
  if (game.round != 0) {
    if (game.timeRemaining != 0) {
      tr.insertCell(4).innerHTML = game.roundLimit + '<br>' + game.round + '<br>' + game.timeRemaining;
    } else {
      tr.insertCell(4).innerHTML = game.roundLimit + '<br>' + game.round;
    }
  } else {
...
function getGameRows(gameNumbers) {
  ...
  var gameNumbers, tournament, privateGame, speedGame, map, gameType, polySlots, initialTroops, playOrder, bonusCards, fortifications, warFog, trenchWarfare, roundLimit, roundLength, round, timeRemaining;
      ...
      try {
        ...
        if (games.getNamedItem('total').nodeValue != '0') {
          for (var gameIndex = 0; gameIndex < data.getElementsByTagName('game').length; gameIndex++) {
            ...
            polySlots = gameData.getElementsByTagName('poly_slots')[0].childNodes[0].nodeValue;
            ...
            trenchWarfare = gameData.getElementsByTagName('trench_warfare')[0].childNodes[0].nodeValue;
            roundLimit = gameData.getElementsByTagName('round_limit')[0].childNodes[0].nodeValue;
            // roundLength = gameData.getElementsByTagName('round_length')[0].childNodes[0].nodeValue; // Unsupported yet
            ...
            game = new gameObject(gameNumber, gameState, tournament, privateGame, speedGame, map, gameType, polySlots, initialTroops, playOrder, bonusCards, fortifications, warFog, trenchWarfare, roundLimit, roundLength, round, timeRemaining, playersArray);
